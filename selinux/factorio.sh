...
if [ $# -eq 1 ]; then
	if [ "$1" = "--update" ] ; then
		time=`ls -l --time-style="+%x %X" factorio.te | awk '{ printf "%s %s", $6, $7 }'`
		rules=`ausearch --start $time -m avc --raw -se factorio`
		if [ x"$rules" != "x" ] ; then
			...
			if [ "$ANS" = "y" -o "$ANS" = "Y" ] ; then
				...
				echo -e "$rules" | audit2allow -R >> factorio.te
...
set -x
make -f /usr/share/selinux/devel/Makefile factorio.pp || exit
make -f /usr/share/selinux/devel/Makefile factorio_init.pp || exit
/usr/sbin/semodule -i factorio.pp
/usr/sbin/semodule -i factorio_init.pp

# Generate a man page off the installed module
sepolicy manpage -p . -d factorio_t
sepolicy manpage -p . -d factorio_init_t
# Fixing the file context on /data/factorio-init/factorio
/sbin/restorecon -F -R -n -v /data/factorio-init
/sbin/restorecon -F -R -n -v /data/factorio
/sbin/restorecon -F -R -n -v /opt/glibc-*
